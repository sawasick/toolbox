...
function generateRows() {
	console.log('生成');
	// column_branch[0] = []; //左から1本目の縦線
	// column_branch[0][0] = [50,2] //上から1本目の横線, に[Y座標,リンク]
	// console.log(column_branch[0]); //[1,50,2]
	// console.log(column_branch[0][0]); //1
	// console.log(column_branch[0][1]);//50
	...
	for (let i = 0; i < columns; i++) {
		...
		for (let j = 0; j < sum; j++) {
			...
			column_branch[i][j] = [pos_Y, 2];
			// console.log(i + 'is' + column_branch[0][i]);
			// console.log(pos_Y);
		}
	}
	drawRows();
...
function drawRows() {
	if (canvas.getContext) {
		const context = canvas.getContext('2d');
		// console.log(column_branch[0].length);
		// console.log(column_branch[0][0]);
		drawColumns();
		// //色を指定する
		// context.fillStyle = 'rgb(00,00,255)'; //枠線の色は青
		//線を描画
		for (let i = 0; i < columns - 1; i++) {
			for (let j = 0, len = column_branch[i].length; j < len; j++) {
				context.fillRect(
					column_iv * (i + 1) + column_w * (i + 1) + column_w,
					column_branch[i][j][0],
					column_iv,
					2
				);
				// console.log(column_branch[i][j][0]);
			}
		}
	}
}
...
document.addEventListener(
	...
	function () {
		...
		btn_generate.addEventListener('click', generateRows2, false);
	},
	false
);
